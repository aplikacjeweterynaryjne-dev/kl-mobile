...
function renderTasks(allTasks) {
    ...
    // ✅ AKTUALIZACJA ZNACZKA (BADGE) NA KAFELKU W MENU GŁÓWNYM
    let tasksToBadgeCount = 0;
    allTasks.forEach(t => {
        const diffMs = today - t.dueDate;
        const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
        // Liczymy niezrobione i nie usunięte w zapomnienie (>14 dni po terminie)
        if (!t.isDone && !(t.isReallyOverdue && diffDays > 14)) {
            tasksToBadgeCount++;
        }
    });

    const badge = document.getElementById('tileTaskBadge');
    if (badge) {
        if (tasksToBadgeCount > 0) {
            badge.style.display = 'flex';
            badge.textContent = tasksToBadgeCount;
        } else {
...
function switchSection(id) { 
    ...
    // Upewnijmy się, że przy powrocie do Zadań są one aktualne
    if (id === 'section-tasks') {
        if (typeof generateAndRenderTasks === 'function') generateAndRenderTasks();
    }
}
